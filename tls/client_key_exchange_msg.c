...
static ssize_t gquic_tls_client_key_exchange_msg_serialize(const void *const msg, void *buf, const size_t size);
static ssize_t gquic_tls_client_key_exchange_msg_deserialize(void *const msg, const void *buf, const size_t size);
...
static ssize_t gquic_tls_client_key_exchange_msg_serialize(const void *const msg, void *const buf, const size_t size) {
    const gquic_tls_client_key_exchange_msg_t *const spec = msg;
    size_t off = 0;
    ...
    __gquic_fill_str_full(buf, &off, &spec->cipher, 3);
    return off;
}

static ssize_t gquic_tls_client_key_exchange_msg_deserialize(void *const msg, const void *const buf, const size_t size) {
    gquic_tls_client_key_exchange_msg_t *const spec = msg;
    size_t off = 0;
    if (msg == NULL || buf == NULL) {
        return -1;
    ...
    if (__gquic_recovery_str_full(&spec->cipher, 3, buf, size, &off) != 0) {
        return -2;
    }
    return off;
}
