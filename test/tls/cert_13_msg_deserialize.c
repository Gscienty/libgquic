...
int main() {
    ...
    gquic_tls_cert_13_msg_t r;
    gquic_tls_cert_13_msg_init(&r);

    printf("%ld\n", gquic_tls_cert_13_msg_deserialize(&r, result, result_size));
    for (i = 0; i < 10; i++)
        printf("%02x ",
               ((unsigned char *) GQUIC_STR_VAL(&r.cert.ocsp_staple))[i]);
    printf("\n");
    for (i = 0; i < 10; i++)
        printf("%02x ",
               ((unsigned char *) GQUIC_STR_VAL(gquic_list_next(GQUIC_LIST_PAYLOAD(&r.cert.scts))))[i]);
    printf("\n");
    for (i = 0; i < 10; i++)
        printf("%02x ",
               ((unsigned char *) GQUIC_STR_VAL(gquic_list_next(GQUIC_LIST_PAYLOAD(&r.cert.certs))))[i]);
    printf("\n");
    for (i = 0; i < 5; i++)
        printf("%02x ",
               ((unsigned char *) GQUIC_STR_VAL(gquic_list_prev(GQUIC_LIST_PAYLOAD(&r.cert.certs))))[i]);
    printf("\n");
