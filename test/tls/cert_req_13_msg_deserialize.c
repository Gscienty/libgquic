...
int main() {
    ...
    *(u_int16_t *) gquic_list_next(GQUIC_LIST_PAYLOAD(&m.supported_sign_algo_cert)) = 0x2234;
    ...
    gquic_tls_cert_req_13_msg_t r;
    gquic_tls_cert_req_13_msg_init(&r);

    printf("%ld\n", gquic_tls_cert_req_13_msg_deserialize(&r, result, result_size));
    printf("%d\n", r.ocsp_stapling);
    printf("%d\n", r.scts);
    printf("%x\n", *(u_int16_t *) gquic_list_next(GQUIC_LIST_PAYLOAD(&r.supported_sign_algo)));
    printf("%x\n", *(u_int16_t *) gquic_list_next(GQUIC_LIST_PAYLOAD(&r.supported_sign_algo_cert)));
    for (i = 0; i < 10; i++)
        printf("%02x ",
               ((unsigned char *) GQUIC_STR_VAL(gquic_list_next(GQUIC_LIST_PAYLOAD(&r.cert_auths))))[i]);
    printf("\n");
